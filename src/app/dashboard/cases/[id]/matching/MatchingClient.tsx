...
import {
  identifyBestExpertProfiles,
  type IdentifyBestExpertProfilesOutput,
} from "@/ai/flows/identify-best-expert-profiles";
      ...
      <div className="space-y-12">
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading
              ? renderSkeletons()
              : profiles?.internalProfiles.map((p) => (
                  <ExpertProfileCard key={p.profileUrl} profile={p} isBestMatch={p === bestMatch} />
                ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading
              ? renderSkeletons()
              : profiles?.externalProfiles.map((p) => (
                  <ExpertProfileCard key={p.profileUrl} profile={p} isBestMatch={p === bestMatch} />
                ))}
          </div>
        </div>
